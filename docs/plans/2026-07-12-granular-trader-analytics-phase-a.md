...
- [x] `packages/store/src/analytics.ts`: `dowHourMatrix(db, f)` → 7×24
...
- [x] Empty cells omitted (client masks); no interpolation anywhere.
- [x] Tests: fixture with trades pinned to known UTC dow/hour; assert exact
      cell values, cross-dow boundary (Sun 23:00 vs Mon 00:00), empty range.
- [x] Export via `packages/store/src/index.ts`.

## Task 2: statistical fields on existing breakdowns

- [x] Extend `tradeBreakdowns` byDow/byHour entries with `se` (standard
      error of mean PnL) and keep `n` (already present as trades count —
      verify field name consistency).
- [x] Tests: hand-computed SE on a 3-trade fixture; single-trade bucket
      (se null, not 0); zero-variance bucket.

## Task 3: `rollingStats` aggregation

- [x] `rollingStats(db, f, windowDays)` → per-day rolling Sharpe (√252
      ...
      capped (e.g. 7-180).
- [x] Emit null (not 0) for days with fewer than windowDays observations —
      the "cannot compute" rule.
- [x] Tests: known 5-day series with hand-computed 3-day window; warmup
      nulls asserted.

## Task 4: `costDecomposition` aggregation

- [x] `costDecomposition(db, f)` → per-month and per-strategy
...
- [x] Tests: fixture deals with nonzero commission/swap; assert gross vs
      net reconciles with existing `realized` totals.

## Task 5: `contributionRanking` aggregation

- [x] `contributionRanking(db, f)` → ranked rows by symbol, by strategy,
      by direction: `{key, net, n, expectancy, winRate, share}` where share
      = net / totalNet (signed).
- [x] Tests: two-symbol fixture; sign handling when total is negative.

## Task 6: API surface

- [x] New `include=` keys on `/performance` in `packages/api/src/rest.ts`:
...
- [x] Route tests: shape assertions + cache-key distinctness + param
      validation (window bounds).

## Task 7: client types + chart primitives

- [x] `apps/web/src/api.ts`: interfaces for the four new payloads.
- [x] Verify vendored `/vendor/echarts.min.js` includes `heatmap`,
      `boxplot`, `radar` + `visualMap` component; if absent, swap in a
      build that does (document the bundle provenance in the file header).
- [x] `components/`: `HeatmapChart` (visualMap diverging red↔green around a
...
- [x] `pages/Edge.tsx` + nav entry in `App.tsx` (Performance group):
...
- [x] Rolling-stability panel (Sharpe + win rate, window selector 30/60/90).
- [x] Panel-level Skeleton/Loadable isolation; react-query keys per include.

## Task 9: existing-view upgrades

- [x] Monthly-returns heatmap on the Calendar view — implemented as tinted
      cells on the existing monthly-returns table (values stay printed,
      YTD kept), reusing the day-cell tint scale rather than a separate
      grid component.
- [x] Equity page: shade `drawdownPeriods` spans onto the equity curve
      (markArea), top-N table already exists — link hover table↔chart.
- [x] Strategies page: cost-decomposition stacked bars (per month) +
      contribution ranked bars with side table (net + expectancy + n).
- [x] Strategy radar panel (moved to the Edge page, not Overview — it sits
      with the other comparison analytics and avoids touching Overview's
      fetch fan-out): win rate, PF, Sharpe, DD⁻¹, expectancy — min-max
      normalized across shown strategies, normalization note in panel hint,
      four most-active strategies.
- [x] PnL box plots per weekday/strategy beside the existing histogram
      (box not violin; n per box).

## Task 10: verification + docs

- [x] `pnpm build:all && pnpm test` green (mise PATH prefix).
